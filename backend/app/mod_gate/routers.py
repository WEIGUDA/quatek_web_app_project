...
from app.mod_gate.utils import normalize_card_number, hid_to_normal
...
def cardtests2():
    ...
    # batch query cards
    card_numbers = list(
        log_collection.find(query_string_dict).distinct("card_number")
    )
    ...
    query_string_dict_updated_with_in_and_test_succeed["card_number"] = {
        "$in": out_logs_card_numbers
    }
        ...
        for log in logs:
            ...
            if in_out_symbol == "进":
                ...
                try:
                    last_in_log = list(
                        filter(
                            lambda in_log: in_log["card_number"]
                            == log["card_number"]
                            and log["test_datetime"] >= in_log["test_datetime"],
                            all_in_logs,
                        )
                    )[-1]
                except:
                ...
                if last_in_log:
                    ...
                    results.append(
                        [
                            ...
                            is_tested,
                            last_in_log["hand"],
                            last_in_log["left_foot"],
                            last_in_log["right_foot"],
                        ]
                    )
            else:
                results.append(
